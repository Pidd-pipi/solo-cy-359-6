...
运营总览页内置「活动打卡与实时排行榜」板块，规则如下：

- 线路详情按顺序展示全部打卡点，队伍必须从第一个点开始依次打卡。
- 同一个打卡点只认第一次有效打卡；跳点、重复提交、活动结束后继续打卡都会被拒绝并返回中文原因。
- 完成全部打卡点后自动生成总用时（最后一次有效打卡时间 − 第一次有效打卡时间）。
- 实时排行榜每 5 秒自动刷新，仅完赛队伍上榜，按总用时从短到长排序。
- 打卡进度与榜单持久化在数据库中，刷新页面后保持一致。

接口一览（前端经 `/api` 前缀访问，由 Nginx / Vite 代理转发）：

| 方法与路径 | 说明 |
| --- | --- |
| `GET /api/overview` | 原有运营总览数据（保留入口） |
| `GET /api/activities` | 活动列表（状态、时间窗、打卡点/队伍数） |
| `GET /api/activities/{id}` | 线路详情：按顺序返回打卡点 |
| `GET /api/activities/{id}/progress` | 各队伍打卡进度与下一个应打卡点 |
| `GET /api/activities/{id}/leaderboard` | 实时排行榜（仅完赛队伍，按总用时升序） |
| `POST /api/activities/{id}/checkins` | 提交打卡，Body：`{"teamId": 1, "checkpointId": 2}`；违反规则返回 `409` 与 `reason` |
